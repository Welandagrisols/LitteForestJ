...
export function DashboardTab() {
  ...
  async function fetchDashboardData() {
    try {
      ...
      const { data: salesData, error: salesError } = await supabase
        ...
        .select(`
          *,
          inventory:inventory_id (plant_name, category),
  ...
  function processBestSellers(sales: any[]) {
    ...
    sales.forEach(sale => {
      const plantName = sale.inventory?.plant_name || 'Unknown'
  ...
  function processCategorySales(sales: any[]) {
    ...
    sales.forEach(sale => {
      const category = sale.inventory?.category || 'Other'
    ...
    <div className="space-y-6 p-4 lg:p-8">
            ...
            <div className="flex items-start justify-between">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-1 text-xs text-emerald-600 dark:text-emerald-400">
                  ...
                  <span className="text-muted-foreground">{inventorySummary.totalQuantity.toLocaleString()} seedlings</span>
                </div>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div className="space-y-2">
                ...
                <p className="text-3xl font-bold tracking-tight">Ksh {salesSummary.thisMonthAmount.toLocaleString()}</p>
                <div className="flex items-center gap-1 text-xs text-blue-600 dark:text-blue-400">
                  <Calendar className="h-3 w-3" />
                  <span className="text-muted-foreground">Last month: Ksh {salesSummary.lastMonthAmount.toLocaleString()}</span>
                </div>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div className="space-y-2">
                ...
                <p className="text-3xl font-bold tracking-tight">{salesSummary.totalSales}</p>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Revenue and number of sales over time</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Top 5 plants by quantity sold</CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
              <div className="h-[250px] flex items-center justify-center text-muted-foreground">Loading...</div>
            ) : bestSellers.length === 0 ? (
              <div className="h-[250px] flex items-center justify-center text-muted-foreground">No sales data yet</div>
            ) : (
                    ...
                    <div key={plant.name} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          {getRankIcon(index)}
                          <span className="font-medium text-sm truncate max-w-[150px]">{plant.name}</span>
                        </div>
                        <div className="text-right">
                          <span className="font-semibold text-sm">{plant.quantity} sold</span>
                          <span className="text-xs text-muted-foreground ml-2">
                            (Ksh {plant.revenue.toLocaleString()})
                          </span>
                        </div>
                      </div>
        ...
        <Card>
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="h-[250px] flex items-center justify-center text-muted-foreground">No customer data yet</div>
            ) : (
              <div className="space-y-4">
                  ...
                  return (
                    <div key={customer.name} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          {getRankIcon(index)}
                          <span className="font-medium text-sm truncate max-w-[150px]">{customer.name}</span>
                        </div>
                        <div className="text-right">
                          <span className="font-semibold text-sm">Ksh {customer.totalSpent.toLocaleString()}</span>
                          <span className="text-xs text-muted-foreground ml-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Revenue breakdown by plant category</CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
              <div className="h-[300px] flex items-center justify-center text-muted-foreground">Loading...</div>
                  ...
                  <TableBody>
                    {lowStockItems.map((item) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell className="text-sm px-4 py-3 font-semibold text-red-600">{item.quantity}</TableCell>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-sm font-medium px-4 py-3">Plant</TableHead>
                    <TableHead className="text-sm font-medium px-4 py-3">Customer</TableHead>
                    <TableHead className="text-sm font-medium px-4 py-3">Qty</TableHead>
                ...
                <TableBody>
                  {recentSales.slice(0, 5).map((sale) => (
                    <TableRow key={sale.id}>
                      ...
                      <TableCell className="text-sm px-4 py-3">{sale.customer?.name || "Walk-in"}</TableCell>
                      <TableCell className="text-sm px-4 py-3">{sale.quantity}</TableCell>
                      <TableCell className="text-sm px-4 py-3 font-semibold text-green-600">Ksh {sale.total_amount.toLocaleString()}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
